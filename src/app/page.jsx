...
import { 
  getAIAnalysisAction, 
  getDailyAccumsAction, 
  saveDailyAccumsAction,
...
// ─── FALLBACK MATCH POOL (Used if no real data/error) ────────────────────────
const FALLBACK_POOL = [
...
function Countdown({ targetMs, color }) {
  const { h, m, s, expired } = useCountdown(new Date(targetMs).getTime());
...
function WinCalc({ totalOdds, color, t }) {
  ...
  return (
    <div style={{padding:"12px 18px",borderBottom:`1px solid ${t.border}`}}>
      <div style={{fontSize:9,color:t.textDim,fontWeight:800,letterSpacing:1.5,textTransform:"uppercase",marginBottom:8}}>💰 WIN CALCULATOR</div>
      <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:t.textDim,marginBottom:4,fontWeight:700}}>STAKE (UGX)</div>
          <input value={stake} onChange={e=>setStake(e.target.value.replace(/\D/g,""))}
            style={{width:"100%",padding:"8px 10px",background:t.bg,border:`1.5px solid ${t.border}`,borderRadius:8,color:t.text,fontFamily:"'Outfit',sans-serif",fontSize:14,fontWeight:700,outline:"none"}}/>
        </div>
        <div style={{color:t.textDim,fontSize:18,paddingBottom:8}}>→</div>
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:t.textDim,marginBottom:4,fontWeight:700}}>WIN (UGX)</div>
...
function PayScreen({ accum, t, dark, onBack, onPaid }) {
  ...
  return (
    <div style={{padding:"18px",animation:"fadeUp 0.3s ease"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:t.textDim,cursor:"pointer",fontSize:13,fontWeight:700,marginBottom:14,padding:0}}>← Back</button>
      <div style={{textAlign:"center",marginBottom:16}}>
        ...
        <div style={{fontSize:12,color:t.textDim,marginTop:3,fontWeight:600}}>{accum.matches.length} picks · Full AI analysis · Valid all day</div>
      </div>
      <div style={{background:t.bg,border:`1px solid ${t.border}`,borderRadius:12,marginBottom:14,overflow:"hidden"}}>
        {accum.matches.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 14px",borderBottom:i<accum.matches.length-1?`1px solid ${t.border}`:"none"}}>
            <div>
              <div style={{fontSize:9,color:t.textDim,fontWeight:800}}>{m.fl} {m.lg}</div>
              <div style={{fontSize:12,fontWeight:800,color:t.text}}>{m.h} vs {m.a}</div>
            </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",padding:"11px 14px",borderTop:`1px solid ${t.border}`,background:`${cfg.color}0C`}}>
          <span style={{fontSize:12,fontWeight:800,color:t.text}}>TOTAL ODDS</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:11}}>
        {[{id:"mtn",lbl:"MTN MoMo",em:"📱",c:"#FFD700"},{id:"airtel",lbl:"Airtel Money",em:"📲",c:"#FF3B5C"}].map(pm=>(
          <button key={pm.id} onClick={()=>setPayMethod(pm.id)} style={{padding:"10px 8px",border:`1.5px solid ${payMethod===pm.id?pm.c:t.border}`,borderRadius:10,cursor:"pointer",background:payMethod===pm.id?`${pm.c}15`:t.surface,display:"flex",flexDirection:"column",alignItems:"center",gap:4,transition:"all 0.2s",position:"relative",fontFamily:"'Outfit',sans-serif"}}>
            ...
            <span style={{fontSize:11,fontWeight:700,color:payMethod===pm.id?pm.c:t.textDim}}>{pm.lbl}</span>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,border:`1.5px solid ${phone.length>=9?cfg.color:t.border}`,borderRadius:10,padding:"11px 13px",background:t.surface,marginBottom:11,transition:"all 0.2s"}}>
        <span style={{color:t.textDim,fontSize:13,whiteSpace:"nowrap",fontWeight:700}}>🇺🇬 +256</span>
        <input value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,"").slice(0,10))} placeholder="7X XXX XXXX"
          style={{flex:1,border:"none",outline:"none",background:"transparent",fontFamily:"'Outfit',sans-serif",fontSize:15,letterSpacing:2,color:t.text,fontWeight:700}}/>
      </div>
...
function AccumCard({ accum, dark, t, initiallyUnlocked=false }) {
  const cfg = TIERS[accum.tier];
  const [unlocked, setUnlocked] = useState(initiallyUnlocked || accum.tier==="free");
  const [payOpen, setPayOpen] = useState(false);
  const [analysisLoading, setAnalysisLoading] = useState(false);
  const { expired } = useCountdown(new Date(accum.firstKick).getTime());

  const handlePaid = (phone) => {
    setUnlocked(true);
    setPayOpen(false);
    // In a real app, we'd verify the backend here. For now, we simulate.
  ...
  return (
    <div style={{background:t.surface,border:`1.5px solid ${unlocked?cfg.color+"66":t.border}`,borderRadius:18,overflow:"hidden",marginBottom:20,animation:"fadeUp 0.5s ease"}}>
      <div style={{background:dark?`${cfg.color}0E`:`${cfg.color}16`,borderBottom:`1px solid ${t.border}`,padding:"13px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:10,color:t.textDim,marginTop:1,fontWeight:800}}>{cfg.desc}</div>
          </div>
        </div>
        {unlocked 
          ? <span style={{background:cfg.color,color:cfg.dark?"#000":"#fff",borderRadius:7,padding:"3px 10px",fontSize:10,fontWeight:900}}>🔓 LIVE</span>
          : <span style={{background:dark?"rgba(255,255,255,0.05)":t.border,color:t.textDim,border:`1px solid ${t.border}`,borderRadius:7,padding:"3px 10px",fontSize:10,fontWeight:800}}>🔒 LOCKED</span>
        }
      </div>
      ...
      <div style={{padding:"9px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:dark?"rgba(0,0,0,0.18)":"rgba(0,0,0,0.02)"}}>
        <span style={{fontSize:9,color:t.textDim,fontWeight:900,letterSpacing:1.5,textTransform:"uppercase"}}>⏳ FIRST MATCH IN</span>
        <Countdown targetMs={accum.firstKick} color={cfg.color}/>
      </div>
        ...
        <>
            ...
            <div key={m.id} style={{borderBottom:`1px solid ${t.border}`,padding:"13px 18px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  <span>{m.flag}</span>
                  <span style={{fontSize:11,color:t.textDim,fontWeight:800}}>{m.league}</span>
                  {m.is_hot&&<span style={{fontSize:9,background:"#FF4D00",color:"#fff",borderRadius:5,padding:"1px 5px",fontWeight:900}}>🔥</span>}
                </div>
                <span style={{fontSize:11,color:t.textDim,fontWeight:700}}>⏰ {new Date(m.kickoff).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <span style={{flex:1,fontWeight:900,fontSize:15,color:t.text}}>{m.home_team}</span>
                <span style={{fontSize:10,color:t.textDim,background:t.border,padding:"2px 8px",borderRadius:5,flexShrink:0,fontWeight:800}}>VS</span>
                <span style={{flex:1,fontWeight:900,fontSize:15,color:t.text,textAlign:"right"}}>{m.away_team}</span>
              </div>
              <div style={{position:"relative"}}>
                <div style={{
                  ...
                  <div>
                    <div style={{fontSize:9,color:t.textDim,letterSpacing:1,textTransform:"uppercase",marginBottom:2,fontWeight:900}}>{m.market}</div>
                  ...
                  <div style={{background:cfg.color,color:cfg.dark?"#000":"#fff",borderRadius:7,padding:"6px 12px",textAlign:"center",minWidth:52}}>
                    ...
                    <div style={{fontSize:8,letterSpacing:1,opacity:0.75,marginTop:1,fontWeight:800}}>ODDS</div>
                  </div>
                </div>
                {!unlocked&&(
                  <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",gap:5,pointerEvents:"none"}}>
                    <span>🔒</span><span style={{fontSize:11,fontWeight:900,color:t.textDim}}>Pay to reveal pick & odds</span>
                  </div>
                )}
              </div>
              {unlocked&&(
                <div style={{marginTop:9,background:dark?"rgba(255,255,255,0.025)":"rgba(0,0,0,0.03)",borderRadius:8,padding:"9px 11px",borderLeft:`2px solid ${cfg.color}55`,animation:"fadeUp 0.4s ease"}}>
                  <p style={{fontSize:12,color:t.textDim,lineHeight:1.65,margin:0,fontWeight:600}}>{m.analysis||"AI Analysis: Form and historical data favor this result strongly."}</p>
                </div>
              )}
            </div>
          ))}
          ...
          <div style={{padding:"14px 18px",background:dark?`${cfg.color}06`:`${cfg.color}0A`,borderBottom:`1px solid ${t.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:12,color:t.textDim,fontWeight:700}}>{accum.matches.length} picks combined</div>
            </div>
          ...
          <div style={{padding:"14px 18px"}}>
            {accum.tier==="free"
              ? <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                  ...
                  <span style={{color:cfg.color,fontWeight:900,fontSize:13}}>Free picks — no payment needed!</span>
                </div>
              : !unlocked
                ? <>
                    ...
                    <p style={{textAlign:"center",fontSize:10,color:t.textDim,marginTop:7,fontWeight:700}}>One-time · Valid all day · No subscription</p>
                  </>
                : <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                    <span style={{color:cfg.color,fontSize:14}}>✅</span>
                    <span style={{color:cfg.color,fontWeight:900,fontSize:13}}>Unlocked — good luck! 🍀</span>
                  </div>
            }
          </div>
        </>
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      // 1. Try to fetch today's accums from Supabase
      const stored = await getDailyAccumsAction();
      if(stored && stored.length > 0) {
        setAccums(stored.reduce((acc, item)=>({...acc, [item.tier]: item}), {}));
      } else {
        // 2. If nothing in DB, generate new ones (this normally happens on a CRON job or by admin)
        // For the demo/launch, we'll generate if empty and save to DB
        const pool = FALLBACK_POOL; // Replace with fetchFixturesAction results when ready
        const newAccums = {};
        const usedIds = [];
        ...
        // 3. Get AI Analysis for the new ones
        for(const tier of ["free","vip","premium"]) {
          const analysis = await getAIAnalysisAction(newAccums[tier]);
          newAccums[tier].matches = newAccums[tier].matches.map(m=>{
        ...
        setAccums(newAccums);
        await saveDailyAccumsAction(newAccums);
      }
    })();
  },[]);
  ...
  return (
    <div style={{background:t.bg,color:t.text,minHeight:"100vh",fontFamily:"'Outfit',sans-serif",fontWeight:600}}>
      {/* NAV */}
      <nav style={{position:"sticky",top:0,zIndex:99,background:dark?"rgba(0,0,0,0.95)":"rgba(242,255,245,0.95)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${t.border}`,padding:"11px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(0,212,94,0.08)",border:"1px solid rgba(0,212,94,0.25)",borderRadius:20,padding:"5px 11px"}}>
            ...
            <span style={{fontSize:11,fontWeight:800,color:"#00D45E"}}>{liveUsers.toLocaleString()}</span>
          </div>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"20px 16px 70px"}}>
        <div style={{textAlign:"center",marginBottom:20,animation:"fadeUp 0.5s ease"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(0,212,94,0.08)",border:"1px solid rgba(0,212,94,0.25)",borderRadius:20,padding:"5px 14px",marginBottom:14}}>
            <span style={{fontSize:11,color:"#00D45E",fontWeight:900,letterSpacing:1}}>📅 {new Date().toLocaleDateString("en-UG",{weekday:"long",day:"numeric",month:"long"})}</span>
          </div>
          <h1 style={{fontFamily:"'Russo One',sans-serif",fontSize:28,color:t.text,letterSpacing:0.5,marginBottom:6}}>AI-Powered <span style={{color:"#00D45E"}}>Tickets</span></h1>
          <p style={{fontSize:13,color:t.textDim,lineHeight:1.6,fontWeight:700}}>researched picks · AI analysis · High accuracy</p>
        </div>
        ...
        <div key={ticker} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,212,94,0.07)",border:"1px solid rgba(0,212,94,0.2)",borderRadius:10,padding:"9px 14px",marginBottom:20,overflow:"hidden",animation:"tickIn 0.4s ease"}}>
          <span style={{fontSize:10,fontWeight:900,color:"#00D45E",letterSpacing:1,whiteSpace:"nowrap"}}>⚡ WIN:</span>
          <span style={{fontSize:12,color:t.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:700}}>
        ...
        {!accums ? (
          <div style={{textAlign:"center",padding:"40px 20px"}}>
            ...
            <div style={{fontFamily:"'Russo One',sans-serif",fontSize:18,color:"#00D45E",marginBottom:8}}>Syncing Daily Picks...</div>
            <div style={{fontSize:13,color:t.textDim,fontWeight:700}}>Connecting to PredictorUG Cloud</div>
          </div>
        ) : (
        ...
        <div style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden",marginTop:20,marginBottom:20}}>
           <div style={{padding:"14px 16px",borderBottom:`1px solid ${t.border}`,fontSize:11,fontWeight:900,color:t.textDim,letterSpacing:1.5}}>🏆 RECENT WINNERS</div>
          {WINS.map((w,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",borderBottom:i<WINS.length-1?`1px solid ${t.border}`:"none"}}>
              <div style={{width:34,height:34,borderRadius:"50%",background:"rgba(0,212,94,0.1)",border:"1px solid rgba(0,212,94,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:13,color:"#00D45E",flexShrink:0}}>{w.u[0]}</div>
              <div style={{flex:1}}>
                <div style={{fontWeight:800,fontSize:13,color:t.text}}>{w.u}</div>
                <div style={{fontSize:11,color:t.textDim,fontWeight:700}}>Stake: UGX {w.s}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontWeight:900,fontSize:14,color:"#00D45E"}}>+{w.w}</div>
                <div style={{fontSize:10,color:t.textDim,fontWeight:700}}>{w.time}</div>
              </div>
            </div>
          ))}
        </div>

        <p style={{textAlign:"center",fontSize:11,color:t.textDim,lineHeight:1.7,fontWeight:600}}>⚠️ Predictions are AI-generated analysis ONLY. Always bet responsibly. 18+ only. PredictorUG is not a bookmaker.</p>
      </div>
    </div>
  );
}
